...
try:
    ...
    print("Warning: ChronoCastMultiModel not available")
    ...
    def _run_forecast(self, run_id):
        ...
        try:
            ...
            if forecast_run.model_type == 'chronocast':
                # Use ChronoCast Multi-Model advanced system
                if not MULTIMODEL_AVAILABLE:
                    raise ValueError("ChronoCastMultiModel is not available. Please install required dependencies.")
                
                try:
                    print(f"Creating ChronoCast Multi-Model system with params: {forecast_run.model_params}")
                    
                    # Prepare data for ChronoCastMultiModel (expects 'ds' and 'y' columns)
                    chronocast_df = data[[dataset.date_column, dataset.target_column]].copy()
                    chronocast_df.columns = ['ds', 'y']
                    
                    # Initialize multi-model system
                    forecast_horizon = forecast_run.model_params.get('forecast_days', 30)
                    chronocast_model = ChronoCastMultiModel(forecast_days=forecast_horizon)
                    
                    # Train all models (XGBoost, ARIMA, SARIMA, Prophet)
                    models_to_use = forecast_run.model_params.get('models', ['xgboost', 'arima', 'sarima', 'prophet'])
                    results = chronocast_model.fit_predict(chronocast_df, models=models_to_use)
                    
                    print(f"ChronoCast Multi-Model trained successfully")
                    print(f"Models trained: {list(results.keys())}")
                    
                    # Get the best model's predictions for evaluation
                    best_model_name, best_mae = chronocast_model.get_best_model()
                    print(f"Best model: {best_model_name} with MAE: {best_mae:.2f}")
                    
                    # Use ensemble predictions if available, otherwise best model
                    if 'ensemble' in chronocast_model.predictions:
                        future_predictions = chronocast_model.predictions['ensemble']
                        print("Using ensemble predictions")
                    else:
                        future_predictions = chronocast_model.predictions.get(best_model_name, [])
                        print(f"Using {best_model_name} predictions")
                    
                    # Store the multi-model system
                    model = chronocast_model
                    
                except Exception as e:
                    print(f"Error creating/fitting ChronoCast Multi-Model: {str(e)}")
                    logger.error(f"ChronoCast Multi-Model error: {str(e)}")
                    raise ValueError(f"ChronoCast Multi-Model error: {str(e)}")
            else:
            ...
            # Handle training time and metrics based on model type
            if forecast_run.model_type == 'chronocast':
                ...
                # Create future predictions data (since ChronoCastMultiModel focuses on future forecasting)
                predictions_data = []
                future_dates = model.get_future_dates(chronocast_df)
                
                for i, (date, pred) in enumerate(zip(future_dates, future_predictions)):
                    predictions_data.append({
                        'index': i,
                        'date': str(date),
                        'actual': None,  # Future predictions don't have actual values
